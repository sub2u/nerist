class Customer < ActiveRecord::Base
  ...
  validates_length_of :email, :within => 5..30
  ...
  def password=(pass)
    @password=pass
    self.password_salt = Customer.random_string(10) if !self.password_salt?
    self.password_hash = Customer.hash_password(@password, self.password_salt)
  end
  private
  protected
  ...
  def self.authenticate(login, pass)
    ...
    return nil if u.nil?
    return u if Customer.hash_password(pass, u.password_salt)==u.password_hash
    nil
  end
end
